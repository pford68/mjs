describe("String.prototype", function(){
    ...
    describe("replaceArgs()", function(){
        it("should replace /{[0-9]+}/ with the corresponding argument", function(){
            var pattern = "The {0} will win the {1} in the {2} season.";
            expect(pattern.replaceArgs('Dallas Cowboys', 'Super Bowl', '2010-11')).toBe("The Dallas Cowboys will win the Super Bowl in the 2010-11 season.");
            expect(pattern.replaceArgs('Texas Rangers', 'World Series', 2010)).toEqual("The Texas Rangers will win the World Series in the 2010 season.");
            try {
                expect("No replacements found".replaceArgs('Texas Rangers', 'World Series', 2010)).toEqual("No replacements found");
            } catch (e) {
            ...
            expect(pattern2.replaceArgs('oris', 'T', 'm', 'ue', ' ')).toEqual(expected);
        })
    ...
    describe("applyTemplate()", function(){
        it ("should replace the /{.+}/ in the string if the contents of {} match a property name of the argument", function(){
            ...
            expect(pattern.applyTemplate( { team: 'Dallas Cowboys', contest: 'Super Bowl', season: '2010-11'} )).toEqual("The Dallas Cowboys will win the Super Bowl in the 2010-11 season.");
            expect(pattern.applyTemplate({ team: 'Texas Rangers', contest: 'World Series', season: 2010})).toEqual("The Texas Rangers will win the World Series in the 2010 season.");
            try {
                expect("No replacements found".applyTemplate({ team: 'Texas Rangers', contest: 'World Series', season: 2010})).toEqual("No replacements found");
            } catch (e) {
                this.fail("A pattern without parameters should not throw an error.");
            }
            ...
            var expected = "Arma virumque cano, Trojiae que primus ob oris";
            expect(pattern2.applyTemplate({beforeFirst: 'oris', first: 'T', second: 'm', third: 'ue', fourth: ' ', fifth: 'ghjkfdslg'})).toEqual(expected);
        });
    });
